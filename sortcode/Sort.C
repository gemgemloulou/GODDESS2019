...
void Sortcode::SortData(Int_t RunNum){
 ...
 char outbuf[128];
  ...
  if(RunNum<74||RunNum==107||RunNum==110||RunNum==289||RunNum==331){
  ...
  TH1F *htheta = new TH1F("theta","theta, sx3",180,0,180);
  TH2F *hEvTh = new TH2F("EvTh","EvTh, Sx3u",180,0,180,5000,0,1000);
  TH1F *hmult = new TH1F("mult","mult",10,0,10);
  ...
  TH1F *huQEx = new TH1F("uQEx","Ex in uQQQ, 400<tdc0<1250, 30P IC cut",500,0,10);
  TH1F *huQExAR = new TH1F("uQExAR","AR Ex in uQQQ, 400<tdc0<1250, 30P IC cut",500,0,50);
  TH2F *hQExTh = new TH2F("QExTh","Ex v Th, uQQQ, ic&tdc",180,0,180,500,0,50);
  TH1F *hSDPEx = new TH1F("SDPEx","Ex, all si",500,0,50);
  TH1F *hSDPuQEx = new TH1F("SDPuQEx","Ex in uQQQ, 400<tdc0<1250, 30P IC cut",500,0,50);
  TH2F *hSDPQExTh = new TH2F("SDPQExTh","Ex v Th, uQQQ, ic&tdc",180,0,180,500,0,50);
  ...
  TH2F *ExEgam1 = new TH2F("ExEgam1","particle Ex vs E gam dop. IC gate. SiMult==1",1000,0,10000,500,0,50);
  TH2F *ExEgamQ = new TH2F("ExEgamQ","particle Ex vs E gam dop for uQQQ only. IC gate",1000,0,10000,500,0,50);
  TH2F *SDPExEgamQ = new TH2F("SDPExEgamQ","particle Ex vs E gam dop for uQQQ only. IC gate",1000,0,10000,500,0,50);
 ...
 // Double_t T1 = 234.; // 8MeV/u, Patrick calculated loss in the target from SRIM - put in routine and do it properly
  ...
  Double_t v1 = TMath::Sqrt((2.0*T1)/m1);
  Double_t vcm = (v1*m1)/(m1+m2);       // velocity of the CoM in the lab
  //Double_t ecm = (0.5*m1*((v1-vcm)*(v1-vcm)))+(0.5*m2*vcm*vcm);
  Double_t ecm = (0.5*m1*TMath::Power(v1-vcm,2.0))+(0.5*m2*TMath::Power(vcm,2.0));
  Double_t E1 = T1+m1; //30P total energy
  Double_t Etot = E1+m2;  //total energy in the lab frame
  Double_t Etotcm = sqrt((m1*m1)+(m2*m2)+2.*E1*m2); 
  Double_t p1 = sqrt(T1*T1+(2.0*m1*T1));
  Double_t beta = p1/Etot; cout << "beta = " << beta << endl;
  Double_t gamma = 1.0/sqrt(1.0-(beta*beta));
  // Double_t Q = m1+m2-m3-TMath::Sqrt((m1*m1)+(m2*m2)+(m3*m3)+(2*m2*E1)-2*E3*(E1+m2)+(2*p1*p)3*TMath::Cos(SiTheta[i]*TMath::Pi()/180.));
 //Ex  = (TMath::Sqrt((Etotcm*Etotcm + m3*m3)-2*(E3*Etot - p1*p3*TMath::Cos(Theta*TMath::Pi()/180.))) - m4);
  Double_t r = 100.; // distance from target to inner barrel 
  
  // AR stuff
  Double_t vel = 0.0;
  Double_t velCoM_sq = 0.0;
  Double_t Ein2 = 0.0;
  ...
  for(Long64_t jentry=0;jentry<nentries;jentry++){
    ...
    for(j=0;j<48;j++){
      ...
      uQTheta[j] = uQEnergy[j] = uQEx[j] = uQfStrip[j] = uQbStrip[j] = 0;
 ...
 if(p30IC->IsInside(goddess->icE,goddess->icDE)){
   // cout << "entry "<<jentry<<" has a hit in the IC" << endl;
   ...
   //*************************************************************
	...
	p3 = TMath::Sqrt((uSX3Energy[uSX3mult]/1000)*(uSX3Energy[uSX3mult]/1000) + (2*m3));
	...
	p3 = TMath::Sqrt((dSX3Energy[dSX3mult]/1000)*(dSX3Energy[dSX3mult]/1000) + (2*m3));
	...
	p3 = TMath::Sqrt((uQEnergy[uQmult]/1000)*(uQEnergy[uQmult]/1000) + (2*m3));
	uQEx[uQmult] = (TMath::Sqrt((Etotcm*Etotcm + m3*m3)-2*(E3*Etot - p1*p3*TMath::Cos(uQTheta[uQmult]*TMath::Pi()/180.))) - m4);

    // try a different method of calculating the q value spectrum
    vel = TMath::Sqrt(2.0*(0.001*uQEnergy[uQmult])/m3);
    velCoM_sq = TMath::Power(vcm,2.0) + TMath::Power(vel,2.0) - 2.0*vel*vcm*TMath::Cos(uQTheta[uQmult]*TMath::Pi()/180.0);
    Ein2 = 0.5*(m3 + TMath::Power(m3,2.0)/m4)*velCoM_sq;
	uQExAR[uQmult] = ecm - Ein2 + 10.0866;
	
	
	// if(iverb) cout << "Inside! E_uQ["<<uQQQ<<"]["<<det.nStrip<<"] = " << E_uQ[uQQQ][det.nStrip] << endl; 
	// add these things to an over Si array... (so index i for the goddess vector)	  
    ...
    SteveEx[uQmult] = ecm-e3cm + 10.0866;
	...
	p3 = TMath::Sqrt((dQEnergy[dQmult]/1000)*(dQEnergy[dQmult]/1000) + (2*m3));
	dQEx[dQmult] = (TMath::Sqrt((Etotcm*Etotcm + m3*m3)-2*(E3*Etot - p1*p3*TMath::Cos(dQTheta[dQmult]*TMath::Pi()/180.))) - m4);
	// if(iverb) cout << "Inside! E_dQ["<<dQQQ<<"]["<<det.nStrip<<"] = " << E_dQ[dQQQ][det.nStrip] << endl; 
	// add these things to an over Si array... (so index i for the goddess vector)	  
	...
	dQmult++;
      ...
      hEx->Fill(Ex[i]);
      hSDPEx->Fill(SteveEx[i]);
      htheta->Fill(SiTheta[i]);
      
    } // end goddess
     if(iverb) cout << "straight after goddess loop, simult = " << SiMult << ", uQ = " << uQmult << endl;
    if(iverb) cout << "after goddess uQE["<<uQmult<<"] = " << uQEnergy[uQmult];
    if(iverb) cout << ", uQTh["<<uQmult<<"] = " << uQTheta[uQmult]; 
	...
	for(q=0;q<uQmult;q++){
	  huQEx->Fill(uQEx[q]);
	  //cout << uQExAR[q] << endl;
	  huQExAR->Fill(uQExAR[q]);
	  //cout << SteveEx[0] << endl;
	  hSDPuQEx->Fill(SteveEx[q]);
	   hQExTh->Fill(uQTheta[q],uQEx[q]);
	    hSDPQExTh->Fill(uQTheta[q],SteveEx[q]);
	...
	for(k=0;k<crysMult;k++){
	  ExEgamQ->Fill(edop[k],uQEx[q]);
	   SDPExEgamQ->Fill(edop[k],SteveEx[q]);
	}
      }
    }
