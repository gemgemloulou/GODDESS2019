...
void ionChamber::Clear() {
  ...
  //x_wire = 0; y_wire = 0; // AR removed these
...
void ionChamber::SetRawValue(UInt_t detChannel, Int_t chType, UInt_t rawValue, Int_t ignoreThresh) {
 ...
 //printf("type anode, rawvalue = %f\n",rawValue);
    ...
    if (ignoreThresh!=0 || (ignoreThresh == 0 && rawValue > threshold)) {
      ...
      //dE = anodeRaw[0];
      //E = anodeRaw[1];  // HLC -- moved dE and E calculation into LoadEvent function
      //if(detChannel==1){ cerr << "rawValue = " << rawValue << ", anodeRaw["<<detChannel<<"] = " << anodeRaw[detChannel] << endl;}
      //if(rawValue > 1){ printf("type anode, rawvalue = %f\n",rawValue);}
    }
...
void ionChamber::LoadEvent(goddessEvent *ev, Bool_t ignoreThresholds) {
  ...
  // cerr << "beginA = " << beginA << ", endA = " << endA << endl;
  //cerr << "beginX = " << beginX << ", endX = " << endX << endl;
  ...
  for (itr2 = anodeRaw.begin(); itr2 != anodeRaw.end(); itr2++) {
    if (itr2 == anodeRaw.begin()) {
      dE = itr2->second;
    } else {
      E += itr2->second;
    }
