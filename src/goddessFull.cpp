...
void goddessFull::ReadConfiguration(TString filename) {
  ...
  while(std::getline(inFile, line)) {
	...
	if (detType == "ion") {
	  ...
	  if ( subType == "t" ) {
	    cout << "Setting TDC thresholds (" << thresholds.size() << ").\n";
	    tdc->SetThresholds(thresholds,1);
	  }
	} else {
	  if (secondaryType) {
...
void goddessFull::ProcessEvent() {
  ...
  for (UInt_t i=0; i < tdcs.size(); i++) {
     tdcs[i].LoadEvent(rawGoddess,0);
     eventOut->tdc0 = tdcs[i].GetTDCValue(0);
     eventOut->tdc1 = tdcs[i].GetTDCValue(1);
     eventOut->tdc2 = tdcs[i].GetTDCValue(2);
     eventOut->tdc3 = tdcs[i].GetTDCValue(3);
     eventOut->tdc4 = tdcs[i].GetTDCValue(4);
     eventOut->tdc5 = tdcs[i].GetTDCValue(5);
 }
